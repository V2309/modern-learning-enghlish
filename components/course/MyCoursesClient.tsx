  ...
  return (
    <div className="container mx-auto px-4 py-10">
      ...
      <div className="mb-10 space-y-2">
        ...
        <p className="text-muted-foreground">
          {courses.length} khóa học · {totalCompleted}/{totalLessons} bài đã hoàn thành
        </p>
      </div>

      {/* Overall progress */}
      {totalLessons > 0 && (
        <div className="mb-10 p-6 rounded-2xl bg-primary/8 border border-primary/20 flex items-center gap-6">
          <div className="h-14 w-14 rounded-2xl bg-primary/15 flex items-center justify-center shrink-0">
            <TrendingUp className="h-7 w-7 text-primary" />
          </div>
          <div className="flex-1 min-w-0 space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-sm font-bold text-foreground">Tiến độ tổng thể</span>
            ...
            <p className="text-xs text-muted-foreground">{totalCompleted} bài xong / {totalLessons} bài tổng</p>
          </div>
        </div>
      )}
          ...
          return (
            ...
            >
              <Link
                ...
                className="group flex flex-col bg-card border border-border rounded-2xl overflow-hidden hover:border-primary/50 hover:shadow-lg hover:shadow-primary/5 transition-all"
              >
                {/* Thumbnail */}
                <div className="relative aspect-video overflow-hidden bg-muted">
                  ...
                  {prog.pct === 100 && (
                    <div className="absolute top-3 right-3 bg-emerald-500 text-white text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full">
                      Hoàn thành
                    </div>
                  )}
                </div>
                ...
                <div className="p-5 flex flex-col flex-1 space-y-3">
                  <div className="flex flex-wrap gap-1.5">
                    <span className={cn('px-2 py-0.5 rounded-md text-[8px] font-black tracking-wider uppercase', level.cls)}>
                      {level.label}
                    </span>
                    {course.subject && (
                      <span className="px-2 py-0.5 bg-muted text-muted-foreground rounded-md text-[8px] font-black tracking-wider uppercase">
                        {course.subject}
                      </span>
                    )}
                  </div>

                  <h3 className="font-extrabold text-foreground group-hover:text-primary transition-colors line-clamp-2 flex-1">
                    {course.title}
                  </h3>
