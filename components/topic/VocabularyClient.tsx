...
export default function VocabularyClient({ initialTopics, userId }: VocabularyClientProps) {
  ...
  const handleAddTopic = async () => {
    ...
    if (res.success && res.topic) {
      // Return value has vocabularies count 0 initially
      const topicWithCount = {
        ...res.topic,
        vocabularies: []
      };
      setTopics((prev) => [topicWithCount, ...prev]);
              ...
              >
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex -space-x-2">
                    {[1, 2, 3].map((n) => (
                      <div key={n} className="h-6 w-6 rounded-full border-2 border-background bg-muted" />
                    ))}
                  </div>
                </div>
              </Link>
