...
export default async function CourseDetailPage(props: PageProps) {
  ...
  if (!user) {
    return <div className="container mx-auto px-4 py-20 text-center text-muted-foreground">Vui lòng đăng nhập để xem chương trình học.</div>;
  }

  const course = await getCourseById(courseId);
  if (!course) {
    return <div className="container mx-auto px-4 py-20 text-center text-muted-foreground">Course not found</div>;
  }
